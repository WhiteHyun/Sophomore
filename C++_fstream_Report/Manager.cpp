...
#pragma region 랜덤중복구별
bool Random_Overlap(const int arr[], const int index)
{
	bool overlap = false;
	for (int i = 0; i < 4; i++)
	{
...
void Manager::Quiz()
{
	if (v.size() == 0)
	{
		cout << "퀴즈를 시작할 수 없습니다!" << endl;
		return;
	}
	srand(time(nullptr));
	int example[4] = { -1, -1, -1, -1 };		//객관식 문제 배열
	int index;			//해당 문제의 벡터 위치
	int answer;		//정답 입력받는 변수
	int example_answer_index;			//정답 보기위치를 지정할 변수
	string _answer;
	int score = 0;
	int i;
	for (i = 0;i < 4 ;i++)
	{
		index = rand() % v.size();
		example_answer_index = rand() % 4;		//랜덤으로 위치를 지정
		example[example_answer_index] = index;	//지정된 랜덤위치에 해당 문제의 수도를 저장
		for (int j = 0; j < 4; j++)
		{
			if (j == example_answer_index)	//만약 정답 인덱스를 참조할 경우
				continue;		//그대로 넘어감

			example[j] = rand() % v.size();	//객관식 랜덤값을 받음
			if (example[j] == index || Random_Overlap(example, j))	//만약 랜덤값이 정답이랑 같거나, 다른 랜덤값이 중복되는 경우
			{
				j--;
				continue;		//다시 랜덤값을 받도록 for문을 돎
			}
		}
		cout << "[문제 " << i + 1 << "] " << v[index].getNation() << "의 수도는?" << endl;
		for (int k = 0; k < 4; k++)	//4개의 객관식 보기 출력
			cout << k + 1 << ") " << v[example[k]].getCapital() << endl;
		cout << "정답 : ";
		while (true) //오류 검출 반복문
		{
			cin >> _answer;
		...
		if (_answer == "9")
			break;
		else if (example[answer - 1] == index)	//정답과 일치하면
			score++;
		cout << endl;
	}
